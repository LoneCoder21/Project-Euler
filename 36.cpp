...
void solve() {
    ll sum = 0;
    for (int i = 1; i < 1e6; ++i) {
        string base2 = (bitset<32>(i)).to_string();
        base2 = base2.substr(find(all(base2), '1') - base2.begin());
        string base10 = to_string(i);
        sum += (base2 == string(base2.rbegin(), base2.rend()) && base10 == string(base10.rbegin(), base10.rend()))*i;
    }
    cout << sum << endl;
}
